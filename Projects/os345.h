...
// Priority Queue task
typedef struct{						// represents a task in the priority queue
	int tid;						// the id of the task
	int priority;					// the priority order of the task
} PQTASK;

const PQTASK emptyTask;		// an empty task
...
typedef struct semaphore			// semaphore
{
	...
	PQueue* bq;							// blocked queue
} Semaphore;

// delta clock entry struct
typedef struct
{
	int time;
	Semaphore* sem;
...
const DCEntry emptyDCEntry;		// a null DCEntry
...
typedef struct							// task control block
{
	...
	void (*sigStopHandler)(void);		// task mySIGSTOP handler
	...
	int tickets;						// task tickets (time) for FSS
} TCB;
