...
extern clock_t myClkTime;					// the clock time of the system
extern Semaphore* semaphoreList;			// linked list of active semaphores
extern jmp_buf reset_context;				// context of kernel stack
extern PQueue* rq;
extern Semaphore* tics10sec;				// ten second semaphore
...
int P2_project2(int argc, char* argv[])
{
	...
	static char* tenSecArgv[] = {"TenSecondArgv","10"};

	printf("\nStarting Project 2");
	SWAP;

	// create the tics10sec task ten times 
	int j;
	for(j = 0; j < 10; j++){
		createTask("tenSec",
					P2_tenSecTask,
...
int P2_listTasks(int argc, char* argv[])
{
	...
	// list all the tasks in the ready queue
	printf("\nReady Queue");
	printQueue(rq);
	// list all the tasks in the blocked queues for each semaphore
...
// Ten Second TASK 
//
int P2_tenSecTask(int argc, char* argv[]){
	while(1){
		char curTime[64];
		SEM_WAIT(tics10sec);
		printf("\nTen Seconds Task: tid = %d\tCurrent time: %s",curTask,myTime(curTime));
		swapTask();
	}
	return 0;
}
